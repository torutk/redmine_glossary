...
Ported to Redmine 3.0.x and 3.1.x on master branch.
It doesn't keep compatibility with 2.x!

Ported to Redmine 4.0.x on support-redmine4 branch.
It doesn't keep compativility with 3.x and below.

Reconstructing(Reimplementing) for Redmine 4.0 on reconstruct branch is in progress.
...
1. Put the downloaded package in the /plugins directory. Or clone from this repository.
2. run the following command to upgrade your database. 
